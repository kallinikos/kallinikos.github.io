...
* Το `<<` μετατοπίζει όλα τα bits του πρώτου αριθμού όσες θέσεις αριστερά λέει ο δεύτερος και εισάγει μηδενικά στα δεξιά. Για παράδειγμα, το $$101$$ \<< $$2$$ θα γίνει $$10100$$. Δοκιμάστε `x = 5 << 2;`.

* Το `>>` κάνει την ίδια δουλειά με το `<<` αλλά αντίστροφα, δηλαδή μετατοπίζει τα bits προς τα δεξιά. Έτσι το $$101$$ \>> $$2$$ θα γίνει $$001$$.
...
Το BIT μπορεί να λύσει το πρόβλημα σε $$O(\log N)$$ ανά query και update με μία προ-επεξεργασία σε $$O(N\log N)$$. Πως όμως δουλεύει αυτή η δομή;
